...
router.delete("/photos/:id", async (req, res): Promise<void> => {
  ...
  if (photo.tripId) {
    ...
    if (remaining.length === 0) {
      ...
      const [trip] = await db.select().from(tripsTable).where(eq(tripsTable.id, photo.tripId));
      ...
      const coverPhotoId = trip?.coverPhotoId === photo.id ? remaining[0].id : trip?.coverPhotoId;
