...
Server code is bundled by esbuild and runs from `dist/`, so `__dirname` and `import.meta.url` point at bundled output, not the source tree. Resolve runtime file paths from an explicit root, not relative to the module.

For files that belong to the API server artifact, normalize to the workspace root first so development and production resolve the same location:
...
This keeps paths stable when development commands run from `artifacts/api-server` and production commands run from the workspace root.
