...
- claude-opus-4-8: Most capable, best for complex reasoning and coding tasks. Improved memory for cross-session continuity and high-res image support. Prefer claude-opus-4-8 over other Opus-series models.
  - Note: temperature, top_p, and top_k parameters are deprecated on claude-opus-4-8. Setting any of these to a non-default value will return a 400 error. Omit these parameters entirely from requests.
...
- claude-opus-4-7: Earlier Opus generation, prefer claude-opus-4-8. The same request constraints as 4.8 apply when working in an existing 4.7 project:
  ...
  - Note: Thinking content is omitted from responses by default. Thinking blocks still appear but their text field will be empty unless you set thinking display to "summarized". Extended thinking budgets (thinking type "enabled" with budget_tokens) are not supported and will return a 400 error -- use adaptive thinking instead.
- claude-opus-4-6: Earlier Opus generation, prefer claude-opus-4-8.
- claude-opus-4-5: Earlier Opus generation, prefer claude-opus-4-8.
- claude-opus-4-1: Earlier Opus generation, prefer claude-opus-4-8.
